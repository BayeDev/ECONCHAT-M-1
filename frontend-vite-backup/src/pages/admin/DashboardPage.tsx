      ...
      {progress !== undefined && (
        <div className="mt-4">
          ...
          <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${progress > 80 ? 'bg-red-500' : progress > 60 ? 'bg-yellow-500' : 'bg-green-500'}`}
    ...
    <div className="p-8">
      ...
      {data?.usage?.today?.percentUsed > 80 && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6 flex items-center gap-3">
          ...
          <p className="text-orange-800">Daily budget is {data.usage.today.percentUsed}% used. Consider adjusting limits.</p>
        </div>
      )}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Bar dataKey="queries" fill="#3b82f6" name="Queries" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-green-600">{data?.users?.byStatus?.ACTIVE || 0}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Pending</span>
              <span className="font-semibold text-yellow-600">{data?.users?.byStatus?.PENDING || 0}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Suspended</span>
              <span className="font-semibold text-red-600">{data?.users?.byStatus?.SUSPENDED || 0}</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                data?.settings?.tier1Enabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }`}>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                data?.settings?.tier2Enabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }`}>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                data?.settings?.signupsEnabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }`}>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{((data?.usage?.successRate || 0) * 100).toFixed(1)}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Avg Latency</span>
              <span className="font-semibold">{(data?.usage?.avgLatency || 0).toFixed(0)}ms</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Total Queries</span>
              <span className="font-semibold">{data?.usage?.today?.queries || 0}</span>
            </div>
          </div>
        </div>
      </div>
